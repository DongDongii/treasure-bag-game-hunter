...
function transformGame(dbGame: DatabaseGame): Game {
  return {
    ...
    supported_platforms: dbGame.supported_platforms || ['PC'], // 默认平台
...
export class SupabaseDatabase implements DatabaseInterface {
  ...
  async createItem(itemData: Omit<Item, 'id' | 'created_at' | 'updated_at'>): Promise<Item> {
    try {
      console.log('🔍 Supabase createItem 开始创建商品:', {
        name: itemData.name,
        game: itemData.game,
      ...
      console.log('📝 即将插入的数据:', {
        ...insertData,
        image: `${insertData.image?.substring(0, 50)}...`
      })
      ...
      if (error) {
        console.error('❌ Supabase 创建商品错误:', {
          code: error.code,
          message: error.message,
          details: error.details,
          hint: error.hint
        })
        throw new Error(`数据库错误 (${error.code}): ${error.message}${error.hint ? ` - ${error.hint}` : ''}`)
      }

      console.log('✅ Supabase 创建商品成功:', data?.id)
  ...
  async getGames(): Promise<Game[]> {
    try {
      ...
      return data.map(game => ({
        ...
        supported_platforms: game.supported_platforms || 'PC',
  ...
  async getGame(id: string): Promise<Game | null> {
    try {
      ...
      return {
        ...
        supported_platforms: data.supported_platforms || 'PC',
        created_at: data.created_at
      }
    } catch (error) {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          supported_platforms: gameData.supported_platforms
        })
        .select()
        .single()
    ...
    try {
      const { data, error } = await supabase
        ...
        .update({
          ...
          ...(gameData.supported_platforms && { supported_platforms: gameData.supported_platforms })
        })
        .eq('id', id)
        .select()
        .single()
